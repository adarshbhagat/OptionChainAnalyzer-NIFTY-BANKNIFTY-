...
file_path = "C:\\Users\\adarsh.sandhu\\Downloads\\Market Data\\Option_Open-interest based analysis\\BANKNIFTY\\"
...
while dt.now().strftime("%H:%M") <= '15:35':
    cache={}
    try:
        data=nsefetch('https://www.nseindia.com/api/option-chain-indices?symbol=BANKNIFTY')
        main=data['records']['data']    
        assumedUnderLyingValue = nse_fno("BANKNIFTY")['underlyingValue']
    except:
        print("An exception occurred in fetching data")
        time.sleep(100)
        continue
    print("**************BANK-NIFTY's Current Value:- "+str(assumedUnderLyingValue)+"**************")
    ...
    put1=(df1[df1['strikePrice'] >= assumedUnderLyingValue]).nsmallest(15,'strikePrice')
    put2=(df1[df1['strikePrice'] <= assumedUnderLyingValue]).nlargest(15,'strikePrice')

    putTotal=put1['changeinOpenInterest'].sum()+put2['changeinOpenInterest'].sum()

    df2=df2[df2['expiryDate']== future_Date]

    call1=(df2[df2['strikePrice'] >= assumedUnderLyingValue]).nsmallest(15,'strikePrice')
    call2=(df2[df2['strikePrice'] <= assumedUnderLyingValue]).nsmallest(15,'strikePrice')
